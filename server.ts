...
app.put('/api/users/:id', (req, res) => {
  ...
  if (username && username.toLowerCase() !== db.users[userIndex].username.toLowerCase()) {
  ...
  writeDb(db);
  res.json({ status: 'success', message: 'User updated successfully.', user: db.users[userIndex] });
});

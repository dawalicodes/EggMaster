...
export default function LoginScreen({ onLoginSuccess }: LoginScreenProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Login error, trying offline fallback bypass:', err);
      // Failover locally if Server is unreachable
      if (username === 'admin' && password === 'admin123') {
        onLoginSuccess({ id: 'admin_user', username: 'admin', name: 'Farm Manager Admin (Offline)', role: 'admin' });
      } else if (username === 'worker' && password === 'worker123') {
        onLoginSuccess({ id: 'worker_user', username: 'worker', name: 'Farm Caretaker Worker (Offline)', role: 'worker' });
      } else {
        setErrorMsg('Network error. Please verify your connection or check your credentials.');
      }
    } finally {
      setLoading(false);
    }
